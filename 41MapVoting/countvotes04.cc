...
void CountVotes::ReadAndCount(Scanner& in_stream)
{
  ...
  while (in_stream.HasNext()) {
    ...
    cout << "CONT X" << test_contest << "X" << endl;
      ...
      // We would like to do this, but we can't.
//    the_votes_.at(test_candidate) += 1;

      // We do this instead.
    the_votes_[test_candidate] += 1;
    cout << "VOTES ARE NOW X" << test_candidate << "X" << the_votes_.at(test_candidate) << endl;
