...
int main(int argc, char *argv[]) {
  ...
  out_stream << kTag << "VECTORA\n" << v_stuff.ToString() << endl;
  out_stream.flush();

  // here's the one part of this code that isn't boilerplate
  // remove a record and then dump data again
  v_stuff.RemoveRecord();
  out_stream << kTag << "VECTORB\n" << v_stuff.ToString() << endl;
  out_stream.flush();
